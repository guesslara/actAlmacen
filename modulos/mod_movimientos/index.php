<?php 	
	...
	//die("<h1 align='center'>Sistema detenido por actualizacion de la Base de Datos.</h1>");	
?>
...
<script language="javascript">
	...
	function crear_movimiento()
	{
		...
		if (as==""||as=="undefined") return;
			...
			if (t==1)
			{
				if (t==1&&(oc==""||oc=="undefined"||oc==null)) { alert("Introduce el numero de Orden de compra"); return; } 
				var ok=validarSiNumero(oc);
				if (!ok) { return; }
			}	
		...
		if (confirm("¿Desea iniciar El Movimiento?"))
		{
			...
			contentType: "application/x-www-form-urlencoded",
			url:"movimientos.php",
			data:"accion=crear_movimiento&t="+t+"&f="+f+"&al="+al+"&r="+r+"&as="+as+"&oc="+oc+"&o="+o,
			beforeSend:function(){ 
	...
	function guarda_producto()
	{
			...
			var datos="accion=insertar_productos&idm="+idm+"&valores="+valoresX;
			//alert(datos);
	...
	function obtener_datos_ids(){
		var n_valor="";
		for(var i7=0;i7<document.frm1.elements.length;i7++){
			if (document.frm1.elements[i7].type=="checkbox"){
				if (document.frm1.elements[i7].checked){
					if (n_valor==""){	
						var n_producto=document.frm1.elements[i7].value;
						var valor_del_id_producto=$("#idp"+n_producto).attr("value");
						//alert("Valor del id producto ("+n_producto+"):"+valor_del_id_producto);
						if (valor_del_id_producto==""||valor_del_id_producto==undefined||valor_del_id_producto==null){ alert ("Error: Introduzca el id del producto."); return; }
						n_valor=n_valor+n_producto+"?"+valor_del_id_producto;
					}else{	
						//n_valor=n_valor+","+document.frm1.elements[i7].value;
						var n_producto=document.frm1.elements[i7].value;
						var valor_del_id_producto=$("#idp"+n_producto).attr("value");
						//alert("Valor del id producto ("+n_producto+"):"+valor_del_id_producto);
						if (valor_del_id_producto==""||valor_del_id_producto==undefined||valor_del_id_producto==null){ alert ("Error: Introduzca el id del producto."); return; }
		...
		if (n_valor==""||n_valor=='undefined'||n_valor==null) { return; }
		var datos="accion=buscar_productos&n_valor="+n_valor;
		...
		data:datos,
		beforeSend:function(){ 
			$("#div_validar_buscar_datos").show().html('<center><br><img src="../img/loading2.gif"><br>Procesando informacion, espere un momento.</center>'); 
		},
		success:function(datos){ $("#div_validar_buscar_datos").show().html(datos); },
		timeout:90000000,
		error:function() { $("#div_validar_buscar_datos").show().html('<center>Error: El servidor no responde. <br>Por favor intente mas tarde. </center>'); }
		});	
...
<style type="text/css">
	...
	#opciones2{ position:relative; width:900px; height:auto; overflow:auto; left:50%; margin-left:-500px; padding:0px; border:#000000 1px solid; }
	...
	.imagenA { width:80px; height:150px; top:25px; float:left; margin-top:25px; margin-left:27px; clear:right; text-align:center; font-size:12px; }
	.contenedor_entradas { position:relative; width:900px; height:180px; float:left; color:#006600; }
	.contenedor_salidas {  position:relative; width:900px; height:180px; float:left; color:#FF0000; }
	/* ESTILOS DE LOS TIPOS DE MOVS */	
...
<body>
	...
	<div id="all">
		...
		<div id="contenido">
			...
			<div id="opciones2">
				...
				<div class="contenedor_entradas">
					<?php 
					$sql_conceptos="SELECT * FROM concepmov WHERE tipo='Ent' AND activo='1' AND nivel='1' ORDER BY orden ASC";
					$result_conceptos=mysql_db_query($sql_db,$sql_conceptos);
				...
				<div class="contenedor_salidas">
					<?php 
					$sql_conceptos="SELECT * FROM concepmov WHERE tipo='Sal' AND activo='1' AND nivel='1' ORDER BY orden ASC";
					$result_conceptos=mysql_db_query($sql_db,$sql_conceptos);
					while($row_conceptos=mysql_fetch_array($result_conceptos))
					{
						?>
			...
			<div id="datos2">
				...
				<div class="n_mov2">
					...
					<div class="campo">
						<input name="txt_tipo_mov1" type="text" class="txt1" id="txt_tipo_mov1" readonly="1" style="width:100px;" />
					...
					<div class="campo">
						<input name="txt_fecha1" type="text" class="txt1" id="txt_fecha1" value="<?=date('Y-m-d')?>" readonly="1" style="width:100px;" />
					...
					<div class="campo">
						<input name="txt_idalmacen1" type="text" class="txt1" id="txt_idalmacen1" value="" readonly="1" style="width:100px;" />
					...
					<div class="campo">
						<input name="txt_idasociado1" type="text" class="txt1" id="txt_idasociado1" value="" readonly="1" style="width:100px;" />
					...
					<div class="campo">
						<input name="txt_referencia1" type="text" class="txt1" id="txt_referencia1" value="" style="width:100px;" />
					</div>	
					
					
					<div class="etiqueta" id="lbl_oc0">Orden de Compra</div>
					<div class="campo" id="txt_oc0">
						<input name="txt_oc1" type="text" class="txt1" id="txt_oc1" value="" style="width:100px;" />
					</div>	
		...
		<div id="div_validar_buscar_datos">&nbsp;</div>
