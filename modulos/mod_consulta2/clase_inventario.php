			...
			?><div id="div_inventario1"><?php			
				...
				*/
				...
				<div id="div_tabla_resultados" style="font-size: 10px;">
					...
					<table align="center" cellpadding="3" cellspacing="0" width="2000" style="font-size:10px;">
					<!--
					<tr>
...
//-->
		...
		$m_almacenes_cex_texto='';						
					...
					if ($res=mysql_query($sql2,$link)){ 
						...
						if($ndr>0){	
							while($reg=mysql_fetch_array($res)){
								//echo "<br>"; 	print_r($reg); #D9FFB3
								?>
								<tr onMouseOver="this.style.background='#CED8F6'" onMouseOut="this.style.background='#ffffff'" style="cursor:pointer;font-size: 10px;" onClick="ver_detalle_producto(<?=$reg["id"]?>)">
									<td align="center" width="auto"><?=$reg["id"]?></td>
									<!--<td><?=$reg["id_prod"]?></td>//-->
									<td width="auto"><?=$reg["descripgral"]?></td>
									<td width="auto"><?=$reg["especificacion"]?></td>
									<td width="auto"><?=$reg["control_alm"]?></td>
									<!--
