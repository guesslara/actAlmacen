...
<?
	...
	GROUP BY id_mov";*/
	$sql="SELECT mov_almacen.*,concepmov.asociado as asociado0, concepmov.tipo, concepmov.concepto ,tipoalmacen.* FROM mov_almacen,concepmov,tipoalmacen WHERE (mov_almacen.almacen=tipoalmacen.id_almacen AND mov_almacen.tipo_mov=concepmov.id_concep) AND mov_almacen.id_mov='".$mov."'";
	...
	if ($asociado0=="Proveedor"){
		
	}
	// PROVEEDOR ...
	if ($asociado0=="Proveedor")
	{
		$sql_proveedor="SELECT id_prov,nr FROM catprovee WHERE id_prov=".$asociado;
		$result_proveedor=mysql_db_query($dbcompras,$sql_proveedor);
		$row_proveedor=mysql_fetch_array($result_proveedor);
		$xasociado="";	
	}
	// ALMACENES
	if (asociado0=="Almacenes")
	{
		$sql_almacen_asociado="SELECT almacen FROM `tipoalmacen` WHERE `id_almacen`=".$asociado;
...
<br /><table width="98%" border="1" align="center" cellspacing="0" cellpadding="1" style="font-size:10px;">
  ...
  <tr>
    ...
    <td class="celda1">Tipo de movimiento</td>
    <td class="celda2">&nbsp;<?= $tipo_mov.".-".$concepto;?></td>
  ...
  <tr>
    ...
    <td width="169" class="celda1">Almacen Operado</td>

    <td width="292" class="celda2">&nbsp;<?=$id_almacen." - ".$almacen;?>

       </td>

    <td class="celda1">Asociado a:</td>

    <td width="322" class="celda2">&nbsp;<?=$asociado0." ".$asociado." ".$xasociado;?></td>
  ...
  <tr>
    ...
    <td class="celda1"><?php if ($concepto=="Compras"){ echo "Orden de Compra (OC):"; } ?></td>

    <td class="celda2">&nbsp;<?php if ($concepto=="Compras"){ echo $oc; } ?></td>
  ...
  <tr>
    ...
    <td colspan="3" class="celda2" >&nbsp;<?=$observ?></td>
