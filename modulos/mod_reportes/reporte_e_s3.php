<?php 
	...
	if ($_GET["action"]=="ejecutar") {
		...
		$sql8="
SELECT 
	mov_almacen.id_mov, mov_almacen.fecha, mov_almacen.tipo_mov, mov_almacen.asociado, concepmov.concepto, 
	concepmov.asociado as asociado0, tipoalmacen.almacen as nalmacen, 
	mov_almacen.almacen, prodxmov.id, prodxmov.id_prod,
	catprod.descripgral, 
	prodxmov.clave, prodxmov.cantidad, prodxmov.cu, prodxmov.cantidad*prodxmov.cu as subtotal
FROM mov_almacen, prodxmov, concepmov, tipoalmacen,catprod
WHERE 
	mov_almacen.id_mov = prodxmov.nummov 
	AND concepmov.id_concep=mov_almacen.tipo_mov 
	AND tipoalmacen.id_almacen=mov_almacen.almacen
...
";
		if ($result8=mysql_db_query($sql_db,$sql8))
		{
		...
		<table width="auto" align="center" cellspacing="0" style="font-family:Verdana, Arial, Helvetica, sans-serif; font-size:12px;">
			<tr style="font-weight:bold; text-align:center; color:#000000;">
			  <td colspan="13" height="20" > <?=$ndr8?> Resultados</td>
			</tr>
			...
			<tr>
			  ...
			  <td><?=$row8["clave"]?></td>
			  <td class="tda"><?=$row8["descripgral"]; ?></td>
